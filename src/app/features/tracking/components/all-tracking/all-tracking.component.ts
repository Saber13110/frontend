...
export class AllTrackingComponent implements OnInit {
  ...
  async startBarcodeScanner(): Promise<void> {
    ...
    try {
      ...
      if (result) {
        this.form.patchValue({ trackingNumber: result });
        this.form.get('trackingNumber')?.markAsTouched();
      }
      */
      ...
      setTimeout(() => {
        this.form.patchValue({ trackingNumber: 'GBX123456789' });
        this.form.get('trackingNumber')?.markAsTouched();
      }, 2000);
    } catch (error) {
  ...
  async trackPackage(event: Event): Promise<void> {
    ...
    if (this.form.get('trackingNumber')?.invalid) {
      this.form.get('trackingNumber')?.markAsTouched();
      return;
    }
    ...
    try {
      ...
      */
      ...
      // Simulation for development
      const { trackingNumber } = this.form.value;
  ...
  async trackByReference(event: Event): Promise<void> {
    event.preventDefault();
    if (
      this.form.get('referenceNumber')?.invalid ||
      this.form.get('selectedCountry')?.invalid
    ) {
      this.form.get('referenceNumber')?.markAsTouched();
      this.form.get('selectedCountry')?.markAsTouched();
      return;
    }
    ...
    try {
      ...
      const { referenceNumber, selectedCountry } = this.form.value;
  ...
  async trackByTCN(event: Event): Promise<void> {
    event.preventDefault();
    if (
      this.form.get('tcnNumber')?.invalid ||
      this.form.get('shipDate')?.invalid
    ) {
      this.form.get('tcnNumber')?.markAsTouched();
      this.form.get('shipDate')?.markAsTouched();
      return;
    }
    ...
    try {
      ...
      const { tcnNumber, shipDate } = this.form.value;
  ...
  async getProofOfDelivery(event: Event): Promise<void> {
    event.preventDefault();
    if (this.form.get('proofNumber')?.invalid) {
      this.form.get('proofNumber')?.markAsTouched();
      return;
    }
    ...
    try {
      ...
      const { proofNumber } = this.form.value;
